...
# Style prompts used when generating via Replicate/Gemini/OpenAI (no Colab).
# Kept in sync with the full 20-style STYLE_PROMPTS in the Colab notebook and
# backend/hf_space/app.py — this used to only have the first 8, so 12 styles
# silently fell back to a generic "modern interior design" prompt whenever
# Colab was disconnected.
STYLE_PROMPTS = {
    ...
    "art_deco": "art deco interior design, geometric sunburst wall panel in black lacquer and gold trim, emerald green velvet armchair, brass and marble side table, fluted glass pendant chandelier, herringbone parquet floor, curved furniture silhouettes",
    "coastal": "coastal interior design, whitewashed shiplap walls, light bleached oak floor, linen slipcovered sofa in soft white, jute rope rug, driftwood accent table, sheer breezy curtains, soft blue and sandy beige palette",
    "french_country": "French country interior design, soft cream limewashed walls, exposed wooden ceiling beams, toile de Jouy upholstered armchair, wrought iron chandelier, weathered oak farmhouse table, powder blue painted cabinetry, terracotta tile floor",
    "farmhouse_rustic": "rustic farmhouse interior design, reclaimed barn wood accent wall, wide plank distressed hardwood floor, black iron light fixtures, open wooden shelving with ceramic crockery, stone fireplace surround, oversized linen sofa, warm cream and charcoal palette",
    "contemporary_glam": "contemporary glam interior design, plush tufted velvet sofa in blush pink, polished chrome and lucite coffee table, oversized crystal chandelier, metallic gold throw pillows, high gloss lacquer cabinetry, faux fur area rug, glossy marble floor",
    "dark_academia": "dark academia interior design, floor to ceiling dark walnut bookshelves with leather bound books, deep forest green velvet wingback armchair, brass library lamp with green glass shade, oxblood leather chesterfield sofa, herringbone dark wood floor",
    "tropical_modern": "tropical modern interior design, natural rattan and woven wicker furniture, abundant monstera and palm plants, warm teak wood accents, open airy layout with louvered shutters, terracotta and cream palette, woven bamboo pendant light",
    "brutalist": "brutalist interior design, raw exposed poured concrete walls and ceiling, monolithic geometric furniture forms, single sculptural black leather chair, minimal charcoal grey palette, exposed structural beams, stark dramatic shadows",
    "mediterranean": "Mediterranean coastal interior design, whitewashed stucco walls, blue and white azulejo tile accents, wrought iron light fixtures, terracotta floor tiles, olive green potted plants, arched doorways",
    "shabby_chic": "shabby chic interior design, distressed white painted furniture, soft pastel floral upholstery, vintage crystal chandelier, weathered wood accents, lace and linen textiles, romantic vintage charm",
    "southwestern_desert": "southwestern desert interior design, warm adobe clay walls, Navajo pattern woven textiles, turquoise accent pillows, leather furniture, cacti in terracotta pots, natural wood beams",
    "memphis_postmodern": "Memphis postmodern interior design, bold primary color blocks, squiggle and geometric patterns, playful asymmetric furniture shapes, terrazzo flooring, chrome and neon accents",
}
...
def build_prompt(style=None, palette=None, custom_prompt=None, colors_only=False, extra_details=None) -> str:
    if colors_only:
        # Mirrors the notebook/hf_space generate_style() colors-only branch:
        # repaint walls/decor to the palette, leave layout and furniture alone.
        prompt = f"only repaint the walls and change decor colors to match the selected palette{ROOM_PRESERVE}{QUALITY_BOOST}, photorealistic, 8k, highly detailed, interior photography"
    elif custom_prompt:
        prompt = f"{custom_prompt}{ROOM_PRESERVE}{QUALITY_BOOST}, photorealistic interior design, high quality, 8k, detailed"
    else:
...
def replicate_generate(image_path, style=None, palette=None, custom_prompt=None, colors_only=False, extra_details=None) -> str:
    ...
    prompt = build_prompt(style, palette, custom_prompt, colors_only, extra_details)
...
def gemini_generate(image_path, style=None, palette=None, custom_prompt=None, colors_only=False, extra_details=None) -> str:
...
def openai_generate(image_path, style=None, palette=None, custom_prompt=None, colors_only=False, extra_details=None) -> str:
...
def generate():
    ...
    # Colors Only mode (StyleSelector.js) sends neither style nor customPrompt —
    # just {colorsOnly: true, palette}. This used to be dropped entirely here,
    # so every Colors Only request 400'd before even reaching Colab/Gemini.
    colors_only = bool(data.get("colorsOnly"))
    ...
    if not style and not custom_prompt and not colors_only:
        return jsonify({"error": "style, customPrompt, or colorsOnly required"}), 400
    if colors_only and not palette:
        return jsonify({"error": "colorsOnly requires a palette"}), 400
        ...
        try:
            resp = requests.post(
                ...
                json={"image": image_b64, "style": style, "palette": palette, "customPrompt": custom_prompt,
                      "colorsOnly": colors_only, "extraDetails": extra_details, "model": model},
    ...
    if GEMINI_API_KEY:
        try:
            image_b64 = gemini_generate(upload_path, style, palette, custom_prompt, colors_only, extra_details)
        except Exception as e:
    ...
    # if OPENAI_API_KEY:
    #     try:
    #         image_b64 = openai_generate(upload_path, style, palette, custom_prompt, colors_only, extra_details)
    #     except Exception as e:
    ...
    #     try:
    #         image_b64 = replicate_generate(upload_path, style, palette, custom_prompt, colors_only, extra_details)
    #     except Exception as e:
...
def add_object():
    ...
    prompt = data.get("prompt", "")
    model = data.get("model", "fast")
    # Only 'library' (catalog cutout, exact paste) is used now — "Add Object
    # From Photo" (the 'reference'/IP-Adapter path) was removed from the UI
    # after repeated reliability issues; Colab now only implements 'library'.
...
@app.route("/redesign-from-reference", methods=["POST"])
def redesign_from_reference():
    # Whole-room style transfer from a reference photo — extracts the reference's
    # style/palette/furniture character via IP-Adapter, unlike /add-object which
    # places one exact item. Colab-only (IP-Adapter needs GPU), same as
    # /furnish-room, /add-object and /delete-object above.
    data = request.json or {}
    image = data.get("image")
    reference_image = data.get("referenceImage")
    palette = data.get("palette")
    model = data.get("model", "fast")
    if not image or not reference_image:
        return jsonify({"error": "image and referenceImage required"}), 400
    if COLAB_URL["url"]:
        try:
            resp = requests.post(
                f"{COLAB_URL['url']}/colab-redesign-from-reference",
                json={"image": image, "referenceImage": reference_image, "palette": palette, "model": model},
                headers=COLAB_HEADERS,
                timeout=300,
            )
            result = resp.json()
...
def preview_styles():
    ...
    data = request.json or {}
    palette = data.get("palette")
    # The user-picked subset from StyleGallery.js (None = every style). This
    # used to be silently dropped, so "preview these 6 styles" generated all
    # 20 at full quality instead — ignoring both the selection and draft mode.
    requested_styles = data.get("styles")
    draft = bool(data.get("draft"))
    # Default "fast" (SD1.5) for the preview loop — "quality" (SDXL) works
    # too but is slow enough across many generations that it's rarely worth it.
    model = data.get("model", "fast")
    ...
    if COLAB_URL["url"]:
        ...
        try:
            resp = requests.post(
                ...
                json={"image": image_b64, "palette": palette, "styles": requested_styles, "draft": draft, "model": model},
                headers=COLAB_HEADERS,
                # Generates several styles in one request, so needs a much
    ...
    # ── Mode 2: Gemini — generate the requested styles SEQUENTIALLY, with a
    #    short pause between calls. Firing them all at once (the old
    #    threading.Thread approach) is exactly what tripped Gemini's rate limit
    #    ("429 too_many_requests") live — APIs like this cap requests per
    #    minute, not just per day. Gemini has no draft/fast-preview mode of its
    #    own, so `draft` has no effect on this path.
    ...
    if GEMINI_API_KEY:
        style_ids = [s for s in (requested_styles or list(STYLE_PROMPTS)) if s in STYLE_PROMPTS]
        if not style_ids:
            return jsonify({"error": "No valid styles requested"}), 400
