...
# A strong IP-Adapter pull (as used for placing a single object in a small,
# tightly cropped region) would overpower the user's own room structure across
# a whole 768x768 scene, so whole-room style transfer needs a gentler pull.
...
# component whose mean confidence is too low. Raised from 0.35 after a wildly wrong,
# low-confidence guess (wood paneling labeled "stairway") still cleared that bar.
...
# that size -- a [150, H, W] tensor for a big photo is gigabytes. The MODEL call itself
# (im, not a resized copy, still goes in below) always cost the same either way --
# SegFormer's own processor already resizes its input to a fixed 512x512 regardless of
# what we pass it, so downsampling the image we feed it couldn't have bought any speed
# there, only risked changing what the model sees. What actually was that slow, and
# safe to shrink without touching the model's own input, is everything AFTER the model
# call: upsampling logits, softmax/argmax, and connectedComponents now run on a
# capped-size copy (max side 1024) instead of the full photo; only the final mask of
...
def detect_objects(image_b64):
    ...
    # SAM returns 3 candidate masks per YOLO box with a quality score each --
    # the code used to always take the best of the 3 with no floor, so a box
    # SAM was genuinely unsure about (all 3 scores low) still produced a
    # "confident-looking" region. IoU is passed explicitly to YOLO's own NMS
    # too, instead of relying on ultralytics' implicit default.
    SAM_MIN_SCORE = 0.5
    YOLO_IOU = 0.5
    # Two YOLO+SAM detections that overlap this heavily are almost always the
    # same physical object detected twice (e.g. "chair" and "sofa" on the same
    # seat) -- the previous IoU dedup only ever compared SegFormer regions
    # against existing items, and only when the label matched exactly.
    DUPLICATE_IOU = 0.7
        ...
        for result in yolo_model(np.array(im), verbose=False, conf=0.2, iou=YOLO_IOU, max_det=40):
            for box in result.boxes:
                ...
                if scores[best] < SAM_MIN_SCORE:
                    continue  # SAM itself wasn't confident in any of its 3 candidates -- skip a spurious segment
                mask, conf = masks[best], float(box.conf.item())
                # YOLO results are confidence-sorted (highest first), so by the time a
                # lower-confidence duplicate of an already-added item shows up here,
                # the better one is already in `items` -- drop the worse one.
...
def generate_style(image_b64, style_name=None, palette=None, custom_prompt=None, colors_only=False,
    ...
    if reference_image:
        # Whole-room style transfer: unlike add_object_from_reference (a single
        # object, strength 0.85, cropped tight), this influences the entire
        # 768x768 scene, so a much gentler IP-Adapter pull is needed or the
        # user's own room structure gets overpowered by the reference photo.
    ...
    with MODEL_LOCK:
        ...
        try:
            ...
            # style_pipe is shared -- always leave it neutral for the next call,
            # whether this one succeeded or raised.
            if ref_pil:
...
# real pixels guarantees the exact item, shape and pose the user picked, unlike a
# generative approximation. Only the object's own silhouette (dilated a bit, for a
# contact shadow) gets
...
@api.post("/redesign-from-reference")
@api.post("/colab-redesign-from-reference")
async def redesign_from_reference_route(request: Request):
    data = await request.json()
    with MODEL_LOCK:
        return finish(generate_style(
...
async def add_object_route(request: Request):
    ...
    # "Add Object From Photo" (IP-Adapter / background-removal path) was removed
    # from the UI after repeated reliability issues -- this route now only
    # serves Furnish Room's "From Library" mode (exact catalog cutout paste).
