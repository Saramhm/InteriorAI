...
const TOOLS = [['style',`${STYLES.length} Design Styles`],['furnish','Furnish Rooms'],['object','Object Editing / Deleting'],['recolor','Object Recolor']];
function AppInner() {
  ...
  const upload=async file=>{
    ...
    let sized,warning;
    try{({file:sized,warning}=await downscaleImage(file));}
    ...
    if(data){const next={image:imageSource(data.image,data.mime_type),image_id:data.image_id,label:'original',width:data.width,height:data.height};
      ...
      if(warning)toast(warning,'info',7000);}
  };
...
const runFurnish = async(prompt, options={})=>{
  ...
  const addObject=async(objectImage,prompt,source)=>{
    if(!current)return;
    ...
    // source ('library' | 'reference') tells the backend explicitly which placement
    // pipeline to use, instead of it having to guess from the image's alpha channel.
    const data=await run('Adding the object…',request=>request('/add-object',{room_image:current.image,object_image:objectImage,prompt:translatedPrompt,selection:sel,source,model:genModel}));
      ...
      {!current?<><div className="page-header"><h1>Transform Your Space</h1><p>Start with your room photo.</p></div><Upload onUpload={upload} busy={!!busy}/></>:<>
        ...
        <div ref={toolPanel} key={`${session}-${tool}`}>
          {tool==='style' && <><img className="current-room" src={current.image} alt="Current room"/>
            <StyleSelector image={current.image} busy={!!busy} onGenerate={fields=>{
              // referenceImage set = whole-room style transfer (IP-Adapter, Colab-only,
              // no Gemini/Replicate fallback) — a different route from plain /generate.
              if(fields.referenceImage)return apply('/redesign-from-reference',fields,'reference_style');
              return apply('/generate',fields,fields.style || (fields.colorsOnly ? 'colors_only' : 'custom_style'));
            }}
