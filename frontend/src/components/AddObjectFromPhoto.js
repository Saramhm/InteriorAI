...
export default function AddObjectFromPhoto({ image, selection, onSelect, busy, onAdd }) {
  ...
  return <section className="tool-panel">
    ...
    {/* 'reference' tells the backend to run IP-Adapter conditioning, not an exact
        paste — `selection` isn't passed here, App.js's addObject() re-derives it
        itself via requestSelection(), so the 3rd argument used to be dropped silently. */}
    <button className="primary-action" disabled={busy || !objectImage || !selection} onClick={() => onAdd(objectImage, prompt.trim(), 'reference')}>Add this object</button>
