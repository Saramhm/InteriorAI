...
const MODES = [
  ...
  ["colors", "Colors Only"],
  ["reference", "From Reference Photo"],
];
...
export default function StyleSelector({ image, busy, onGenerate, onPreview, onUsePreview, onExploreAll }) {
  ...
  const canApply =
    ...
    mode === 'reference' ? !busy && !!referenceImage :
  ...
  const apply = () => {
    ...
    if (mode === 'reference') return onGenerate({ referenceImage, palette });
  ...
  return <section className="tool-panel">
    ...
    {mode === 'reference' && <>
      <p>Upload a photo of a room whose look you like — the system reads its overall style, color palette and furniture character and applies that to your own room, while keeping your room's actual walls, windows and layout.</p>
      <button className="upload-zone" disabled={busy} onClick={() => refInput.current.click()}
        onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); chooseReference(e.dataTransfer.files[0]); }}>
        {referenceImage
          ? <img className="reference-preview" src={referenceImage} alt="Reference room" />
          : <div className="upload-idle"><h2>Upload reference room photo</h2><p>Drop a JPG, PNG or WEBP here, or choose a file.</p></div>}
      </button>
      <input ref={refInput} type="file" accept="image/jpeg,image/png,image/webp" hidden disabled={busy}
        onChange={e => { chooseReference(e.target.files[0]); e.target.value = ''; }} />
    </>}
